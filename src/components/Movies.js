...
slope[maxGain.index].color = colors.success;
slope[maxGain.index].strokeWidth = '3px';
slope[maxLoss.index].color = colors.failure;
slope[maxLoss.index].strokeWidth = '3px';
...
const Movies = () => (
  <div className="chartContainer">
    ...
    <ResponsiveXYFrame
      ...
      axes={[
        { className: 'slopeTick', orient: 'left', tickFormat: d => <text textAnchor="end" style={{fill: getColor(d-1, sortedMovies09, '2018')}}>{getName(d-1, sortedMovies09).substr(0,40)}</text>, ticks: 100},
        { className: 'slopeTick', orient: 'right', tickFormat: d => <text style={{fill: getColor(d-1, sortedMovies18, '2009')}}>{getName(d-1, sortedMovies18).substr(0,40)}</text>, ticks: 100},
