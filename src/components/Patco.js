...
console.log(getRandTrains(10, 10));

function getRandTrains(total, totalStops) {
  var trains = [];
  for (var i=0; i<total; i++) {
    trains.push(getRandTrain(i, totalStops));
  }
  return trains;
}

function getRandTrain(id, total) {
  var times = [];
  for (var i=0; i<total; i++) {
    times.push(getRandTime());
  }
  return {
    id: id,
    times: times
...
function getRandTime() {
  return `${chance.hour()}:${chance.minute()}`;
}
...
class Patco extends React.Component {
  constructor(props) {
    super(props);
    this.display = {};
    this.process();
  ...
  render() {
    return (
      <div className="chartContainer">
        <h1>Patco Schedule Reimagined</h1>
        <h3>Precision vs Readability</h3>
        <ResponsiveXYFrame
          { ...sharedProps }
